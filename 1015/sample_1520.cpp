...
vector<vector<int>> board, dp; //board: 위치 정보, dp: 탐색 유무 정보
int n, m;

int dfs(int cr, int cc) { //인자: 시작 위치
    if (cr == n - 1 && cc == m - 1) //오른쪽 아래 끝까지 왔다면 경로가 하나 생긴 것이므로 dp값 1로 초기화 후 리턴
        return dp[cr][cc] = 1; //
    if (dp[cr][cc] != -1) //이미 값(현재 정점에서 오른쪽 아래 끝까지 가는 경로의 수)이 존재한다면
        return dp[cr][cc]; //현재 위치 dp 반환

    dp[cr][cc] = 0; //탐색 시작을 위해 0으로 초기화
    for (int i = 0; i < 4; i++) { //상하좌우 이동 가능
        int nr = cr + dr[i]; //상or하 이동
        int nc = cc + dc[i]; //좌or우 이동
        //보드의 범위 안에 있고, 현재 정점의 값보다 작을 경우
        if (nr >= 0 && nr < n && nc >= 0 && nc < m && board[cr][cc] > board[nr][nc]) //범위 벗어나지 않았고 위치가 올바르다면
...
int main() {
    ...
    board.assign(n, vector<int>(m, 0)); //행렬 정보 초기화
